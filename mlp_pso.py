...
config.train_data_cost_label, config.test_data_cost_label = train_test_split(
    train_data, train_data_label, test_size=0.3, random_state=0)

best_particle, cost_best_particle = pyswarm.pso(
    func=cost.cost, lb=np.array([0.00001, 0.00001]), ub=np.array([0.99999, 0.99999]),
    swarmsize=10, debug=True, minstep=-1, minfunc=-1)

output = config.model[(best_particle[0], best_particle[1])].predict(test_data)

...
def pso(func, lb, ub, ieqcons=[], f_ieqcons=None, args=(), kwargs={},
    ...
    obj = lambda *args: func(*args, **kwargs)
    ...
    for i in range(S):
        ...
        if i==0:
            g = p[0, :].copy()
            config.bestg_model[0] = copy.deepcopy(config.model[i])
    ...
    ax.scatter(plotData[:, 0], plotData[:, 1], plotData[:, 2], c='b', marker='.')
    plotData = np.zeros((S, 3))
    ii=0
    ...
    while it<=maxiter:
        ...
        ax.scatter(plotData[:, 0], plotData[:, 1],
                   plotData[:, 2], c='b', marker='.')
        plotData = np.zeros((S, 3))
        ii = 0
        it += 1

    print('Stopping search: maximum iterations reached --> {:}'.format(it))

    if not is_feasible(g):
        print("However, the optimization couldn't"
              " find a feasible design. Sorry")
    ax.scatter(g[0], g[1], fg, c='r', marker='o')
    ax.set_xlabel('Momentum term')
    ax.set_ylabel('Learning rate')
    ax.set_zlabel('Error of MLP_PSO on the train dataset')
